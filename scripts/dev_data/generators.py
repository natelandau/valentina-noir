    ...
    for company in companies:
        for _ in range(cfg.num_campaigns):
            campaign = await Campaign.create(
                company=company,
                name=fake.campaign_title(),
                description=fake.campaign_description(),
            )
            ...
            for b in range(cfg.books_per_campaign):
                book = await CampaignBook.create(
                    campaign=campaign,
                    number=b + 1,
                    name=fake.book_title(),
                    description=fake.book_text(),
                )
                books.append(book)
                for c in range(cfg.chapters_per_book):
                    chapter = await CampaignChapter.create(
                        book=book,
                        number=c + 1,
                        name=fake.chapter_title(),
                        description=fake.chapter_text(),
                    )
...
async def create_characters(
    ...
    for campaign in campaigns:
        company = companies_by_id[str(campaign.company_id)]  # type: ignore[attr-defined] # ty:ignore[unresolved-attribute]
        company_users = users_by_company[str(campaign.company_id)]  # type: ignore[attr-defined] # ty:ignore[unresolved-attribute]
        for i in range(cfg.num_characters):
            ...
            # The autogeneration handler leaves biography empty; fill it for dev data.
            character.biography = fake.character_biography()
            await character.save(update_fields=["biography"])
    ...
    for character in characters:
        for _ in range(_count(cfg.inventory_per_char)):
            await CharacterInventory.create(
                ...
                description=fake.inventory_item_description(),
...
async def create_dice_rolls(
    ...
    for character in characters:
        company_users = users_by_company[str(character.company_id)]  # type: ignore[attr-defined] # ty:ignore[unresolved-attribute]
        ...
        company_id = _uuid(character.company_id)  # type: ignore[attr-defined] # ty:ignore[unresolved-attribute]
        character_id = _uuid(character.id)
        campaign_id = _uuid(character.campaign_id) if character.campaign_id else None  # type: ignore[attr-defined] # ty:ignore[unresolved-attribute]
...
async def create_notes(
    ...
    for campaign in campaigns:
        for _ in range(_count(cfg.notes_per_target)):
            await Note.create(
                company_id=_uuid(campaign.company_id),  # type: ignore[attr-defined] # ty:ignore[unresolved-attribute]
                user=_author(campaign.company_id),  # type: ignore[attr-defined] # ty:ignore[unresolved-attribute]
                campaign=campaign,
                title=fake.note_title(),
                content=fake.note_body(),
            )
    for book in books:
        campaign = campaigns_by_id[str(book.campaign_id)]  # type: ignore[attr-defined] # ty:ignore[unresolved-attribute]
        for _ in range(_count(cfg.notes_per_target)):
            await Note.create(
                company_id=_uuid(campaign.company_id),  # type: ignore[attr-defined] # ty:ignore[unresolved-attribute]
                user=_author(campaign.company_id),  # type: ignore[attr-defined] # ty:ignore[unresolved-attribute]
    ...
    for character in characters:
        for _ in range(_count(cfg.notes_per_target)):
            await Note.create(
                company_id=_uuid(character.company_id),  # type: ignore[attr-defined] # ty:ignore[unresolved-attribute]
                user=_author(character.company_id),  # type: ignore[attr-defined] # ty:ignore[unresolved-attribute]
                character=character,
                title=fake.note_title(),
                content=fake.note_body(),
            )
    ...
    async def _attach(*, company_id: UUID, parent_kwarg: str, parent: object) -> None:
        ...
        for _ in range(_count(cfg.images_per_target)):
            ...
            await S3Asset.create(
                ...
                **{parent_kwarg: parent},  # type: ignore[arg-type] # ty:ignore[invalid-argument-type]
            )

    for character in characters:
        await _attach(company_id=character.company_id, parent_kwarg="character", parent=character)  # type: ignore[attr-defined] # ty:ignore[unresolved-attribute]
    for book in books:
        campaign = campaigns_by_id[str(book.campaign_id)]  # type: ignore[attr-defined] # ty:ignore[unresolved-attribute]
        await _attach(company_id=campaign.company_id, parent_kwarg="book", parent=book)  # type: ignore[attr-defined] # ty:ignore[unresolved-attribute]
    for chapter in chapters:
        parent_book = books_by_id[str(chapter.book_id)]  # type: ignore[attr-defined] # ty:ignore[unresolved-attribute]
        campaign = campaigns_by_id[str(parent_book.campaign_id)]  # type: ignore[attr-defined] # ty:ignore[unresolved-attribute]
        await _attach(company_id=campaign.company_id, parent_kwarg="chapter", parent=chapter)  # type: ignore[attr-defined] # ty:ignore[unresolved-attribute]
...
async def build_api_keys(developers: list[Developer]) -> list[DevApiKey]:
    ...
    for developer in developers:
        ...
        company_permissions = [
            (p.company_id, p.permission)  # type: ignore[attr-defined] # ty:ignore[unresolved-attribute]
        ...
        keys.append(
            DevApiKey(
                ...
                company_permissions=company_permissions,  # type: ignore[arg-type] # ty:ignore[invalid-argument-type]
            )
        )
    return keys
