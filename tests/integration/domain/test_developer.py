...
async def test_developer_new_key(
    ...
    db_developer = await Developer.get(id=mirror_global_admin.id)
    ...
    assert json_data["username"] == mirror_global_admin.username
    assert json_data["email"] == mirror_global_admin.email
