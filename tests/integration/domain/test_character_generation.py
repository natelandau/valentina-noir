...
class TestAutogenerateCharacter:
    ...
    async def test_autogenerate_character(
        ...
        assert str(character.company_id) == str(session_company.id)
        assert str(character.user_creator_id) == str(session_user_storyteller.id)
        assert str(character.user_player_id) == str(session_user_storyteller.id)
        assert str(character.campaign_id) == str(session_campaign.id)
...
class TestChargenSessions:
    ...
    async def test_list_chargen_sessions_excludes_other_users(
        ...
        for s in sessions:
            assert s["user_id"] == str(session_user.id)
    ...
    async def test_start_chargen_creates_session_document(
        ...
        assert str(session.user_id) == str(session_user.id)
        assert str(session.company_id) == str(session_company.id)
        assert str(session.campaign_id) == str(session_campaign.id)
