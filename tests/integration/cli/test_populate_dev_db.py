...
class TestPopulateData:
    ...
    async def test_populate_data_generates_full_dataset(self) -> None:  # noqa: PLR0915
        ...
        # Then: campaigns, books, and chapters carry generated body text
        new_campaigns = await Campaign.filter(company_id=new_companies[0].id)
        ...
        assert new_chapters
        assert all(ch.description and "\n\n" in ch.description for ch in new_chapters)

        # Then: generated characters carry a biography
        new_chars = [c for c in chars if str(c.company_id) == str(new_companies[0].id)]
        assert new_chars
        assert all(c.biography for c in new_chars)

        # Then: content entities are present (floors pinned to 1 in cfg above)
        assert await CharacterInventory.all().count() >= 1
        # Then: every generated inventory item carries a description
        new_inventory = await CharacterInventory.filter(character_id__in=[c.id for c in new_chars])
        assert new_inventory
        assert all(item.description for item in new_inventory)
