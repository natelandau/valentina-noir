...
from vapi.db.models import Campaign, Character, CharacterTrait, Trait, TraitCategory, User
...
from vapi.lib.exceptions import ConflictError, NotEnoughXPError, ValidationError
...
class TestCharacterTraitService:
    """Test the character trait service."""

    async def test_is_safe_increase(
        ...
        service._is_safe_increase(character_trait, 1)

        # Then the trait value should be increased
        await character_trait.sync()
        assert True  # no exception raised

    async def test_is_safe_increase_over_max_value(
        self, character_trait_factory: Callable[[dict[str, ...]], CharacterTrait]
    ) -> None:
        """Verify that the is_safe_increase method works."""
        service = CharacterTraitService()
    ...
    async def test_is_safe_decrease(
        self, character_trait_factory: Callable[[dict[str, ...]], CharacterTrait]
    ) -> None:
        ...
        trait = await Trait.find_one(Trait.is_archived == False)
        character_trait = await character_trait_factory(value=trait.max_value, trait=trait)
        service._is_safe_decrease(character_trait, 1)
        assert True  # no exception raised

    async def test_is_safe_decrease_below_min_value(
        self, character_trait_factory: Callable[[dict[str, ...]], CharacterTrait]
    ) -> None:
        """Verify that the is_safe_decrease method works."""
        service = CharacterTraitService()
    ...
    class TestAddConstantTraitToCharacter:
        ...
        async def test_add_constant_trait_to_character(
            ...
            result = await service.add_constant_trait_to_character(character, trait.id, 2)
        ...
        async def test_add_constant_trait_to_character_over_max_value(
            ...
            with pytest.raises(ValidationError):
                await service.add_constant_trait_to_character(
                    character, trait.id, trait.max_value + 1
                )
        ...
        async def test_add_constant_trait_to_character_below_min_value(
            ...
            with pytest.raises(ValidationError):
                await service.add_constant_trait_to_character(
                    character, trait.id, trait.min_value - 1
                )
    ...
    class TestCreateCustomTrait:
        ...
        async def test_create_custom_trait(
            ...
            service = CharacterTraitService()
            result = await service.create_custom_trait(character, dto)
        ...
        async def test_create_custom_trait_conflict(
            ...
            with pytest.raises(ConflictError):
                await service.create_custom_trait(character, dto)
    ...
    class TestChangeCharacterTraitValue:
        ...
        async def test_increase_character_trait_value(
            ...
            # Given a character and trait
            character_trait = await character_trait_factory(value=0)
            spy_after_save = mocker.spy(CharacterTraitService, "after_save")

            # When we increase the trait value
            service = CharacterTraitService()
            result = await service.increase_character_trait_value(character_trait, 1)
        ...
        async def test_increase_character_trait_value_over_max_value(
            self, character_trait_factory: Callable[[dict[str, ...]], CharacterTrait], mocker: Any
        ) -> None:
            """Verify a validation error is raised if the trait value is increased above the max value."""
            # Given a character and trait
            character_trait = await character_trait_factory(value=5)

            # When we increase the trait value
            service = CharacterTraitService()
            with pytest.raises(ValidationError):
                await service.increase_character_trait_value(character_trait, 1)
        ...
        async def test_decrease_character_trait_value(
            ...
            service = CharacterTraitService()
            result = await service.decrease_character_trait_value(character_trait, 1)
        ...
        async def test_decrease_character_trait_value_below_min_value(
            self,
            character_trait_factory: Callable[[dict[str, ...]], CharacterTrait],
            mocker: Any,
        ) -> None:
            """Verify a validation error is raised if the trait value is decreased below the min value."""
            # Given a character and trait
            character_trait = await character_trait_factory(value=0)
            service = CharacterTraitService()
            with pytest.raises(ValidationError):
                await service.decrease_character_trait_value(character_trait, 1)
        ...
        async def test_purchase_trait_value_with_xp(
            ...
            character = await character_factory(
                user_player_id=target_user.id, campaign_id=campaign.id
            )
            character_trait = await character_trait_factory(value=1, character_id=character.id)

            # When we purchase the trait value with xp
            service = CharacterTraitService()
            result = await service.purchase_trait_value_with_xp(character, character_trait, 1)
        ...
        async def test_purchase_trait_value_with_xp_not_enough_xp(
            ...
            with pytest.raises(NotEnoughXPError):
                await service.purchase_trait_value_with_xp(character, character_trait, 1)

        async def test_purchase_trait_value_over_max_value(
            self,
            character_factory: Callable[[dict[str, ...]], Character],
            character_trait_factory: Callable[[dict[str, ...]], CharacterTrait],
            campaign_factory: Callable[[dict[str, ...]], Campaign],
            ...
            service = CharacterTraitService()
            with pytest.raises(ValidationError):
                await service.purchase_trait_value_with_xp(character, character_trait, 100)
        ...
        async def test_refund_trait_value_with_xp(
            ...
            service = CharacterTraitService()
            result = await service.refund_trait_value_with_xp(character, character_trait, 1)
        ...
        async def test_refund_trait_value_with_xp_below_min_value(
            self,
            character_factory: Callable[[dict[str, ...]], Character],
            character_trait_factory: Callable[[dict[str, ...]], CharacterTrait],
            campaign_factory: Callable[[dict[str, ...]], Campaign],
            ...
            service = CharacterTraitService()
            with pytest.raises(ValidationError):
                await service.refund_trait_value_with_xp(character, character_trait, 1)
        ...
        async def test_purchase_trait_increase_with_starting_points(
            ...
            trait = await Trait.find_one(Trait.is_archived == False)
            character_trait = await character_trait_factory(
                value=trait.min_value, trait=trait, character_id=character.id
            )
            spy_is_safe_increase = mocker.spy(CharacterTraitService, "_is_safe_increase")
            spy_after_save = mocker.spy(CharacterTraitService, "after_save")

            # When we purchase the trait value with starting points
            service = CharacterTraitService()
            result = await service.purchase_trait_increase_with_starting_points(
                character=character, character_trait=character_trait, num_dots=1
            )
            ...
            spy_after_save.assert_called_once_with(ANY, result)
            spy_is_safe_increase.assert_called_once_with(ANY, character_trait, 1)
            await character.sync()
        ...
        async def test_purchase_not_enough_starting_points(
            ...
            # Given a character and trait
            character = await character_factory(starting_points=0)
            character_trait = await character_trait_factory(value=1, character_id=character.id)
        ...
        async def refund_trait_decrease_with_starting_points(
            ...
            spy_after_save.assert_called_once_with(ANY, result)
            spy_is_safe_decrease.assert_called_once_with(ANY, character_trait, 1)
