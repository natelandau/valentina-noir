...
class TestPurgeDBExpiredItems:
    ...
    async def test_purge_db_expired_objects(
        ...
        mocker.patch("vapi.lib.postgres_database.init_tortoise", return_value=None)

        old_date = time_now() - timedelta(days=60)

        character_old_not_archived = await pg_character_factory(is_archived=False)
        character_old_archived = await pg_character_factory(is_archived=True)
        character_new_not_archived = await pg_character_factory(is_archived=False)
        character_new_archived = await pg_character_factory(is_archived=True)

        # Bypass auto_now to set old date_modified directly via filter update
        await PgCharacter.filter(id=character_old_not_archived.id).update(date_modified=old_date)
        await PgCharacter.filter(id=character_old_archived.id).update(date_modified=old_date)
        ...
        for char in [
            ...
            assert await PgCharacter.filter(id=char.id).first() is not None

        # Then old archived characters are deleted
        assert await PgCharacter.filter(id=character_old_archived.id).first() is None

    async def test_purge_db_expired_audit_logs(
        self,
        pg_developer_factory: Callable[..., Any],
        mocker: MockerFixture,
    ) -> None:
...
class TestPurgeS3AssetsErrorIsolation:
    ...
    async def test_s3_per_asset_error_does_not_block_others(
        self,
        pg_s3asset_factory: Callable[..., Any],
        ...
        # Create company and user explicitly so pg_s3asset_factory has a valid company FK
        company = await pg_company_factory()
        user = await pg_user_factory(company=company)

        asset_1 = await pg_s3asset_factory(is_archived=True, company=company, uploaded_by=user)
        asset_2 = await pg_s3asset_factory(is_archived=True, company=company, uploaded_by=user)
...
class TestPurgeTemporaryCharacters:
    ...
    async def test_purge_old_temporary_characters(
        ...
        old_temp = await pg_character_factory(
            is_temporary=True,
            is_chargen=False,
        )
        old_date = time_now() - timedelta(hours=48)
        await PgCharacter.filter(id=old_temp.id).update(date_modified=old_date)

        # When we run the task
        await purge_db_expired_items({})

        # Then the old temporary character is deleted
        assert await PgCharacter.filter(id=old_temp.id).first() is None
    ...
    async def test_keep_recent_temporary_characters(
        ...
        recent_temp = await pg_character_factory(
            is_temporary=True,
            is_chargen=False,
        )
        ...
        assert await PgCharacter.filter(id=recent_temp.id).first() is not None
    ...
    async def test_keep_chargen_temporary_characters(
        ...
        chargen_char = await pg_character_factory(
            is_temporary=True,
            is_chargen=True,
        )
        old_date = time_now() - timedelta(hours=48)
        await PgCharacter.filter(id=chargen_char.id).update(date_modified=old_date)

        # When we run the task
        await purge_db_expired_items({})

        # Then the chargen character is NOT deleted (chargen session cleanup handles these)
        assert await PgCharacter.filter(id=chargen_char.id).first() is not None
    ...
    async def test_keep_non_temporary_characters(
        ...
        regular_char = await pg_character_factory(
            is_temporary=False,
            is_chargen=False,
        )
        ...
        assert await PgCharacter.filter(id=regular_char.id).first() is not None
    ...
    async def test_purge_temporary_character_cleans_up_traits(
        ...
        temp_char = await pg_character_factory(
            is_temporary=True,
            is_chargen=False,
        )
        trait_record = await pg_character_trait_factory(
            character=temp_char,
            value=3,
        )
        ...
        old_date = time_now() - timedelta(hours=48)
        await PgCharacter.filter(id=temp_char.id).update(date_modified=old_date)

        # When we run the task
        await purge_db_expired_items({})

        # Then both the character and its trait are deleted
        assert await PgCharacter.filter(id=temp_char.id).first() is None
        assert await PgCharacterTrait.filter(id=trait_record.id).first() is None
