...
class CampaignNoteController(BaseNoteController):
    ...
    async def get_campaign_note(self, *, campaign: Campaign, note: Note) -> dto.NoteResponse:
        """Get a campaign note by ID."""
        return await self._get_note(note, campaign.id)
    ...
    async def update_campaign_note(
        self, campaign: Campaign, note: Note, data: dto.NotePatch
    ) -> dto.NoteResponse:
        """Update a campaign note by ID."""
        return await self._update_note(note, campaign.id, data)
