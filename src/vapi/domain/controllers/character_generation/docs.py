...
Begin a character generation session by generating multiple character options.  The number of characters generated is controlled by the company's `character_autogen_num_choices` setting.

If a single character is generated, it is automatically marked as active and the user is not required to select it.

If multiple characters are generated, the user must select one of the characters to keep. This can be done by calling the `.../chargen/finalize` endpoint with the selected character ID.  **Note:** No character is activated until it has been finalized.
...
**Access:**
* The user must have enough experience points to complete the action.
"""
