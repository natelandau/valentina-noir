...
class ChargenSessionResponse(msgspec.Struct):
    ...
    def from_model(cls, session: "ChargenSession") -> "ChargenSessionResponse":
        ...
        return cls(
            id=session.id,
            user_id=session.user_id,  # type: ignore[attr-defined] # ty:ignore[unresolved-attribute]
            campaign_id=session.campaign_id,  # type: ignore[attr-defined] # ty:ignore[unresolved-attribute]
