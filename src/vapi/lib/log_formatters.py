"""Log formatters and the structured key=value message parser.

Holds binary-content sanitization, the standard/color/JSON formatters, and a
``StructuredMessageParser`` that turns ``key=value`` log messages into typed
fields. ``log_config`` composes these into the application logging config.
"""
...
class StructuredMessageParser:
    """Parse ``key=value`` pairs out of a log message into typed Python values."""

    def parse(self, message: str) -> dict[str, Any]:
        """Extract structured key=value pairs from a message.
...
class LitestarJsonFormatter(JsonFormatter):
    """JSON formatter that extracts key=value pairs from log messages into structured fields."""
